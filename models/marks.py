"""
Marks Model - CRUD operations and calculations for Marks entity
"""
...
class Marks:
    ...
    def update_marks(mark_id: int, marks_obtained: int, max_marks: int = 100, 
        ...
        query = """
        UPDATE Marks 
        SET marks_obtained = %s, max_marks = %s, assessment_date = %s, assessment_type = %s 
        WHERE mark_id = %s
        """
    ...
    def get_class_analytics(class_name: str, section: str = None) -> dict:
        ...
        if section:
            student_condition = "s.class = %s AND s.section = %s"
            params = (class_name, section)
        else:
            student_condition = "s.class = %s"
            params = (class_name,)
    ...
    @staticmethod
    def get_subject_analytics(subject_id: int) -> dict:
        """Get analytics for a specific subject"""
        query = """
        SELECT s.name, s.class, s.section, m.marks_obtained, m.max_marks
        FROM Marks m
        JOIN Student s ON m.student_id = s.student_id
        WHERE m.subject_id = %s
        ...
        results = fetch_all(query, (subject_id,))
        ...
        total_obtained = sum(result[3] for result in results)
        total_max = sum(result[4] for result in results)

        average_marks = total_obtained / total_students
        average_percentage = Marks.calculate_percentage(total_obtained, total_max)

        marks_list = [result[3] for result in results]
        highest_marks = max(marks_list)
        lowest_marks = min(marks_list)

        # Calculate pass/fail based on 40% threshold
        pass_count = sum(1 for result in results 
                        if Marks.calculate_percentage(result[3], result[4]) >= 40)
